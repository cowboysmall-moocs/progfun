...
class Empty extends TweetSet {
    ...
    def mostRetweeted = throw new java.util.NoSuchElementException("Empty contains no Tweets")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
    ...
    def mostRetweeted: Tweet = {
        ...
        maxOfTweetSet(left, maxOfTweetSet(right, elem))
    }

    def maxOfTweetSet(ts: TweetSet, tweet: Tweet): Tweet = {

        if (!ts.isInstanceOf[Empty]) max(ts.mostRetweeted, tweet) else tweet
    }
